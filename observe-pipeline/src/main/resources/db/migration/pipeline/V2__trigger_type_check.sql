-- B8: 给 executions / failed_executions 的 trigger_type 加 CHECK 约束（含 UNKNOWN 兜底值）。
--
...
ALTER TABLE failed_executions ADD CONSTRAINT ck_failed_executions_trigger_type
    CHECK (trigger_type IN ('CDC', 'CRON', 'API', 'UNKNOWN'));
