...
**ErrorType（执行失败分类）**:
`DefaultPipelineRunner.classify` 当前产出 5 值：`SCRIPT_COMPILATION`/`SCRIPT_SANDBOX`/`SCRIPT_TIMEOUT`/`SCRIPT_EXECUTION`/`NODE_EXECUTION`（+ 兜底 `UNKNOWN`）。另两值 `PIPELINE_TIMEOUT`/`GRACEFUL_SHUTDOWN_KILL` 是**预留**——为"pipeline 整体超时"与"worker 关闭时强 kill 在途 pipeline 记失败"两个未来能力占位，当前 `classify` 不产出（DDL CHECK 仍允许，待能力实现时启用）。
_Avoid_: 把 PIPELINE_TIMEOUT/GRACEFUL_SHUTDOWN_KILL 当死值删——它们是预留待实现，非废弃。
